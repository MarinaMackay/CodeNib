...
- TestAgentCallsBM25SearchE2E: real LLM (Vertex AI Gemini) + real BM25 index,
  full round-trip.
...
class TestAgentSelectsBM25Search:
    ...
    We verify that the routing agent correctly wires the plan to the real
    executor and that the executor is actually called.
    """
    ...
    def test_route_selects_bm25_search(self, loaded_bm25_search):
        """route() must include bm25_search in the returned invocations."""
        llm = _make_llm_returning(self._plan_for_bm25_search("BM25CodeIndexer"))
    ...
    def test_invocation_has_correct_query(self, loaded_bm25_search):
        """The query field in the invocation must match what was passed to route()."""
        query = "parse_config"
    ...
    def test_executor_is_called_with_invocation_inputs(self, loaded_bm25_search, mock_bm25):
        """
        After routing, running the executor_fn with the invocation inputs must
        call the underlying BM25 indexer.
        """
        query = "HTTPClient"
...
# ---------------------------------------------------------------------------
# TestAgentCallsBM25SearchE2E
# ---------------------------------------------------------------------------


@pytest.mark.slow
class TestAgentCallsBM25SearchE2E:
    """
    Full round-trip: real LLM decides to use bm25_search,
    real BM25 index executes the query, results are returned.

    BM25 index is cached at /tmp/codeminer_bm25_e2e_index (reused across runs).
    Set CODEMINER_BM25_INDEX_PATH to override.

    Uses a dedicated SkillRegistry to avoid interference from the autouse
    clean_registry fixture.
    """

    @pytest.fixture(autouse=True)
    def clean_registry(self):
        """Override the module-level autouse fixture -- no-op for this class."""
        yield

    @pytest.fixture(scope="class")
    def bm25_env(self):
        """Build BM25 index, load skills, create LLM, return (agent, registry)."""
        ...
        llm = LiteLLMChat(
            ...
            temperature=0.0,
            max_tokens=1024,
            extra_kwargs={"vertex_project": "ucsd-cse-stable-gcp"},
        )
        agent = SkillRoutingAgent(llm=llm, registry=registry)
