...
from codeminer.agent.tools import search_code_snippets
...
class TestSearchCodeSnippets(unittest.TestCase):
    ...
    def test_search_file_path(self):
        ...
        result = search_code_snippets(
            search_terms=["codeminer/search.py"],
    ...
    def test_search_multiple_terms(self):
        ...
        result = search_code_snippets(
            search_terms=["BM25CodeIndexer", "search"],
            repo_path=self.repo_path
        )
        self.assertIsInstance(result, str)
        self.assertGreater(len(result), 0, "Search result should not be empty")
    ...
    def test_search_file_entity_function(self):
        ...
        self.assertIsInstance(result, str)
        self.assertGreater(len(result), 0, "Search result should not be empty")
        self.assertIn("search_code_snippets", result, "Result should contain the function name")
        self.assertIn("def search_code_snippets", result, "Result should contain function definition")
        self.assertIn("Entity:", result, "Result should indicate it's an entity search")
        self.assertNotIn("#!/usr/bin/env python3", result, "Should not return entire file content")
    ...
    def test_search_file_entity_class(self):
        ...
        self.assertIsInstance(result, str)
        self.assertGreater(len(result), 0, "Search result should not be empty")
        self.assertIn("TestSearchCodeSnippets", result, "Result should contain the class name")
        self.assertIn("class TestSearchCodeSnippets", result, "Result should contain class definition")
        self.assertIn("Entity:", result, "Result should indicate it's an entity search")
