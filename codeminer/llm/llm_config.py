...
class LLMConfig:
    ...
    def _get_vertex_project(self) -> str:
        """Get the Google Cloud project ID for Vertex AI.

        Checks (in order):
        1. config_data / env ``VERTEX_PROJECT``
        2. Service account JSON via ``GOOGLE_APPLICATION_CREDENTIALS``
        3. ``google.auth.default()`` (ADC — covers ``gcloud auth``, GCE metadata, etc.)
        """
        # Explicit project override
        project = self.get_config_value_optional("VERTEX_PROJECT")
        if project:
            return project

        # Try service account JSON (if provided)
        ...
        raise LLMConfigurationError(
            "Could not determine Google Cloud project for Vertex AI. "
            "Set VERTEX_PROJECT, GOOGLE_APPLICATION_CREDENTIALS, "
