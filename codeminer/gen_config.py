...
def get_llm(**kwargs) -> LLM:
    ...
    codeminer_config: Config = kwargs.get("codeminer_config", None)
    model = kwargs.get("model", None)
    if model.startswith("claude"):
        # first check if the provider has been set
        if codeminer_config.provider == "vertexanthropic":
            print(f"Using AnthropicVertex model: {model}")
            service_account_path = os.path.expanduser(
                codeminer_config["VERTEX_SERVICE_ACCOUNT_PATH"]
            )
            ...
            try:
                ...
                kwargs["region"] = codeminer_config["VERTEX_REGION"]
                LLM_func = VertexAnthropicWithCredentials
            except Exception as e:
                raise Exception(f"gen_config: Failed to get vertexanthropic LLM") from e
        else:
            kwargs["api_key"] = codeminer_config["ANTHROPIC_API_KEY"]
            LLM_func = Anthropic
    elif model.startswith("gpt"):
        kwargs["api_key"] = codeminer_config["OPENAI_API_KEY"]
        LLM_func = OpenAI
    elif model.startswith("gemini"):
        # Load Google Cloud credentials
        service_account_path = codeminer_config["VERTEX_SERVICE_ACCOUNT_PATH"]
    ...
    # delete codeminer_config from kwargs
    if "codeminer_config" in kwargs:
        del kwargs["codeminer_config"]
