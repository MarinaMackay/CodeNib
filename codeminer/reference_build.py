...
class ReferenceVisitor(ast.NodeVisitor):
    ...
    def visit_FunctionDef(self, node):
        ...
        if self.current_class:
            full_function_name = f"{self.current_file}::{self.current_class}::{function_name}"
        ...
        self.current_scope = self.current_file if not prev_function else \
                           (f"{self.current_file}::{prev_function}" if not self.current_class else 
                            f"{self.current_file}::{self.current_class}::{prev_function}")
    ...
    def _handle_attribute_call(self, base_name, method_name):
        ...
        # debug
        logger.debug(f"Resolved method: {resolved_method}, base_name: {base_name}, method_name: {method_name}, scope: {self.current_scope}, file: {self.current_file}")
        ...
        for class_info in self.symbol_table.classes.values():
            if method_name in class_info['methods']:
                ...
                self.graph.add_edge(caller, method_path, edge_type="possible_method_call")
                ...
                if file.endswith(".py"):
                    ...
                    with open(file_path, "r") as f:
                        try:
                            ...
                            self.symbol_tables[rel_file_path] = symbol_builder.symbol_table
                        except SyntaxError:
            ...
            for file in files:
                if file.endswith(".py"):
                    ...
                    with open(file_path, "r") as f:
                        try:
                            tree = ast.parse(f.read())
                            symbol_table = self.symbol_tables.get(rel_file_path, SymbolTable())
...
def build_graph(repo_path: str) -> nx.DiGraph:
    ...
    for root, _, files in os.walk(repo_path):
        for file in files:
            if file.endswith(".py"):
                file_path = os.path.join(root, file)
                rel_file_path = os.path.relpath(file_path, repo_path)  # Keep repo_path for relative paths
                ...
                try:
                    with open(file_path, "r") as f:
                        ...
                        for func_name, func_path in symbol_builder.symbol_table.functions.items():
                        ...
                        for class_name, class_info in symbol_builder.symbol_table.classes.items():
                            class_node = f"{rel_file_path}::{class_name}"
                            graph.add_node(class_node, type="class")
                            graph.add_edge(rel_file_path, class_node, edge_type="contains")
                            
                            for method_name, method_path in class_info['methods'].items():
                                graph.add_node(method_path, type="method")
                                graph.add_edge(class_node, method_path, edge_type="contains")
    ...
    for file_path, symbol_table in reference_builder.symbol_tables.items():
        ...
        if file_path.startswith('my_project'):
            ...
            print("  Functions:")
            for func_name, func_path in symbol_table.functions.items():
                print(f"    - {func_name}")
    ...
    method_calls = [(u, v) for u, v, d in graph.edges(data=True) 
