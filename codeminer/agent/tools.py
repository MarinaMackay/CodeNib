...
from .utils import find_matching_files_from_list, get_all_python_files, is_entity_definition, extract_entity_content, search_entity_occurrence
...
def search_code_snippets(
    ...
    if line_nums and file_path_or_pattern:
        result += _search_by_line_numbers(line_nums, file_path_or_pattern, repo_path, include_files)
...
def _search_by_line_numbers(line_nums: List[int], file_path: str, repo_path: str, include_files: List[str]) -> str:
    """
    ...
    try:
        ...
        for line_num in line_nums:
            if 1 <= line_num <= len(lines):
                ...
                result += f'Line {line_num} is out of range (file has {len(lines)} lines)\n\n'
