...
void write_python_source(const std::filesystem::path& project_root) {
    ...
    if (ec) {
        throw std::runtime_error("Failed to create sample directory: " + ec.message());
    }

    const auto module_path = sample_dir / "module.py";
    std::ofstream python_file(module_path);
    if (!python_file.is_open()) {
        throw std::runtime_error("Failed to create sample module at " + module_path.string());
    }
...
std::filesystem::path write_test_index(const std::filesystem::path& project_root) {
...
int main(int argc, char** argv) {
    ...
    const std::string method_symbol = "sample/module.py:SampleClass.sample_method()";
